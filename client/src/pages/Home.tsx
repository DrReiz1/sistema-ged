  ...
  return (
    <div className="space-y-4 md:space-y-6 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs md:text-sm text-gray-500 mt-1">Bem-vindo ao sistema de gestão de documentos da TSEA</p>
      </div>
      ...
      <motion.div
        ...
        className="rounded-xl bg-white border border-gray-200 shadow-sm p-4 md:p-6"
      >
        ...
        <form onSubmit={handleSearch} className="flex gap-2 md:gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-10 md:h-11 pl-10 pr-4 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-700 focus:outline-none focus:border-[#FF201A] focus:ring-1 focus:ring-[#FF201A]/20 transition-all"
            />
          </div>
          <button type="submit" className="h-10 md:h-11 px-4 md:px-6 rounded-lg bg-[#FF201A] text-white text-sm font-semibold hover:bg-[#e01a14] transition-colors border border-[#bf0f0c] whitespace-nowrap">
            Buscar
          </button>
        </form>
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4 sm:grid-cols-4">
        {[
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
            className="rounded-xl bg-white border border-gray-200 shadow-sm p-3 md:p-5 flex items-center gap-3 md:gap-4">
            <div className={`h-8 md:h-10 w-1.5 rounded-full flex-shrink-0 ${s.color}`} />
            <div className="min-w-0">
              <p className="text-xl md:text-2xl font-bold text-gray-800">{s.value}</p>
              <p className="text-[10px] md:text-xs text-gray-400 mt-0.5 leading-tight">{s.label}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-3">
              ...
              {recent.map((doc) => {
                ...
                return (
                  <Link key={doc.id} href={`/documents/${doc.id}`}>
                    <div className="flex items-center gap-3 px-4 md:px-5 py-2.5 md:py-3 hover:bg-gray-50 cursor-pointer transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs md:text-sm font-medium text-gray-700 truncate">{doc.title}</p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className="hidden sm:inline text-[11px] text-gray-400">{type?.name}</span>
                      </div>
                    </div>
                  </Link>
                );
        ...
        {/* Right column */}
        <motion.div className="space-y-3 md:space-y-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
          {/* Favoritos */}
          <div className="rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">

                  ...
                  <Link key={doc.id} href={`/documents/${doc.id}`}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${statusColor[doc.status]}`}>
                          {statusLabel[doc.status] ?? doc.status}
                        </span>
                        <span className="hidden sm:inline text-xs text-gray-400">{type?.name}</span>
                      </div>
                    </div>
                  </Link>
                );
